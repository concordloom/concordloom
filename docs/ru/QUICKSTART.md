...
Concord Loom управляет ограниченными системами циклов. Фреймворк не требует
конкретного жизненного цикла ПО, движка процессов, поставщика моделей,
платформы размещения или структуры репозитория. Это руководство предлагает три
первых маршрута:

1. изучить общий сайт и принятую self-binding самого Concord Loom;
2. обнаружить свидетельства в своём репозитории, не исполняя его код;
3. пройти полную цепочку артефактов на примере generic SDLC.

После установки основные команды Concord Loom работают только с локальными
файлами. Они не обращаются к поставщику моделей или облачному сервису. Публичный
сайт — статическая проекция только для чтения, а не часть переносимого runtime.
...
## 2. Изучите общий сайт и self-binding
...
Активный корневой цикл называется `concord-change`. Поставка ПО — одна из
возможных предметных связок, а не граница фреймворка. На сайте концептуальное
объяснение отделено от проекции живой системы.

Из исходного checkout проверьте и запустите те же статические файлы:
...
Self-binding демонстрирует самоприменение, но не самоавторизацию. Дочерние циклы
`observe`, `negotiate`, `bind`, `execute`, `verify`, `publish` и `evolve`
ограничены активной политикой. `evolve` может создать предложение преемника, но
не получает права его активировать.
...
## 6. Скомпилируйте и отдельно активируйте binding

Компиляция создаёт registry и предложение binding:
...
Ни компиляция, ни создание каталога не могут молча заменить активную binding.
...
Создайте запуск для корневого цикла скомпилированного registry:
...
binding, registry, policy, candidate, repository, actor и evidence, необходимые
этому переходу:
...
показывает принятую структуру этой binding и записанные факты запуска. Он не
...
Команде нужны активная binding, policy, сигналы, явные операции, риск, автор
...
ПО и self-binding Concord Loom.

## 10. Проверьте готовый пример поставки ПО

Помеченный тегом checkout содержит все артефакты generic SDLC в согласованной
