...
конфигурация. Продвигайте карточку командами:
...
План становится фактом только после записи попытки: кто исполнял работу, какую
модель и инструкции использовал, какими инструментами пользовался, обращался ли
к сети, какие внешние изменения выполнил, сколько времени потратил и чем
закончилась работа. Запуск завершён, только когда `run complete` выполнилась
успешно.
...
конфигурация определяет, кто проверяет и включает следующую. Эволюция может
предложить себе замену, но не может разрешить её включение.
...
- [Руководство по Атласу](ATLAS.md) объясняет интерактивное и автономное представления.
