...
Если зависимости нет, программа запуска и предварительная проверка выводят
`install_plan` для соответствующего выпуска v0.1. План сначала выбирает
`pipx`, затем `uv tool`, затем отдельное виртуальное окружение вне целевого
репозитория. Покажите план оператору и получите разрешение до выполнения всех
команд. Затем снова запустите `--resolve`.
