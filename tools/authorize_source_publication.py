#!/usr/bin/env python3
"""Propose, then separately activate, self-binding v6 publication authority."""
...
PREDECESSOR_DIR = SOURCE / "v5"
GRAPH_DIR = SOURCE / "v3"
TARGET = SOURCE / "v6"
STAMP = "2026-07-27T18:00:00Z"
OPERATOR = {
    "id": "example-operator",
    "kind": "operator",
...
NODES = [
    ...
    ("maintain-repository-presence", "release-distribution", "Maintain Repository Presence", "Оформлять репозиторий",
     ...
     "publisher", "release"),
    ("publish-source-change", "release-distribution", "Publish Source Change", "Публиковать изменение исходников",
     "Publish a pinned candidate to a branch and pull request.", "Публиковать зафиксированную версию в ветку и запрос на слияние.",
     "publisher", "release"),
    ("accept-source-change", "release-distribution", "Accept Source Change", "Принимать изменение исходников",
     "Merge an independently reviewed source change by a separate operator act.", "Объединять независимо проверенное изменение отдельным решением оператора.",
     "operator only", "operator"),
    ("maintain-organization-presence", "release-distribution", "Maintain Organization Presence", "Оформлять организацию",
     "Publish the organization mark, name and website.", "Публиковать знак, название и сайт организации.",
     "publisher", "release"),
...
DETAILS = {
    ...
    "publish-source-change": ("A pinned candidate, independent review and publication authority.", "A branch and pull request bound to the candidate digest.", "Зафиксированная версия, независимая проверка и право публикации.", "Ветка и запрос на слияние, связанные с хешем версии.", ["candidate-manifest", "branch-ref", "pull-request-receipt"]),
    "accept-source-change": ("A reviewed pull request and exact candidate digest.", "A separate operator merge receipt.", "Проверенный запрос на слияние и точный хеш версии.", "Отдельное решение оператора и отчёт о слиянии.", ["review-receipt", "operator-decision", "merge-receipt"]),
    "maintain-organization-presence": ("Approved organization identity and brand assets.", "An organization profile settings receipt.", "Принятые данные организации и материалы бренда.", "Отчёт о настройках профиля организации.", ["mark", "organization-metadata", "settings-receipt"]),
...
def development_model(binding_base_digest: str) -> dict[str, object]:
    ...
    return {
        ...
        "schema_version": "0.1",
        "id": "concordloom-development-system-v6",
...
def main() -> None:
    ...
    policy["id"] = "concordloom-self-policy-v6"
    ...
    if "accept-source-publication" not in operator_role["capabilities"]:
        operator_role["capabilities"].append("accept-source-publication")
        operator_role["capabilities"].sort()
    ...
    design_proposal = propose_loop_design(
        ...
        decisions,
        policy,
        proposal_id="concordloom-development-system-v6-proposal",
    ...
    design = accept_loop_design(
        ...
        decision_id="accept-concordloom-development-system-v6",
        actor=OPERATOR,
        accepted_at=STAMP,
        authority_ref="operator",
        rationale=(
            "Accept the operator-approved source publication, source merge, "
            "and organization presence cycles."
        ),
    )
    registry = compile_registry(
        ...
        registry_id="concordloom-development-registry-v6",
    )
    ...
    merge_scope = {
        **release_scope,
        "external_mutations": ["github-pull-request-merge"],
    }
    organization_scope = {
        **release_scope,
        "external_mutations": ["github-organization-profile"],
    }
    live_scope = {
        "read_paths": ["."],
        "write_paths": [],
    ...
    for edge in registry["containment_graph"]["edges"]:
        ...
        elif child_id == "publish-source-change":
            edge["grant"]["scope"] = deepcopy(source_scope)
        elif child_id == "accept-source-change":
            edge["grant"]["scope"] = deepcopy(merge_scope)
        elif child_id == "maintain-organization-presence":
            edge["grant"]["scope"] = deepcopy(organization_scope)
    ...
    signals = [
        {
            ...
            "schema_version": "0.1",
            "id": "self-use-found-missing-source-publication-authority",
            "base_binding_digest": base_digest,
            "category": "coverage",
            "severity": "warning",
            "occurrences": 2,
            "summary": (
                "A fully governed self-hosted run cannot publish its pinned "
                "source candidate or pull request under the v5 effects."
            ),
            "source_digest": digest(
                {"source": "run-audit:2026-07-27:source-publication-gap"}
            ),
        ...
        },
        {
            "kind": "concordloom.evolution-signal",
            "schema_version": "0.1",
            "id": "organization-migration-exposes-unowned-identity-effect",
            "base_binding_digest": base_digest,
            "category": "friction",
            "severity": "warning",
            "occurrences": 1,
            "summary": (
                "Moving the repository to the Concord Loom organization made "
                "organization avatar and profile publication a distinct effect."
            ),
            "source_digest": digest(
                {"source": "conversation:2026-07-27:organization-migration"}
            ),
            "provenance": [{"kind": "evidence", "ref": "operator-migration"}],
        },
    ]
    evolution = propose_evolution(
        ...
        [
            {
                "op": "add",
                "target_kind": "loop",
                "target_id": "concordloom-development-system-v6",
                "value": {
                    "root_loop_id": "steward-concordloom",
        ...
        expected_effect=(
            "Add exact source publication, operator merge, and organization "
            "profile effects without broadening executors or reviewers."
        ),
        risk={
            "level": "medium",
            "failure_modes": [
                "Source publication could leak into an executor or reviewer.",
                "Organization identity could be changed without publisher authority.",
            ],
            "rollback": (
                "Reactivate the v5 predecessor if the v6 publication model fails "
                "independent review."
            ),
        },
        generated_at=STAMP,
        policy=old_policy,
        proposal_id="authorize-source-publication-and-organization-profile",
    )
    ...
    paths = {
        ...
        "loop_design_proposal": (
            "framework/concordloom/v6/loop-design-proposal.json"
        ),
        "accepted_loop_design": "framework/concordloom/v6/loop-design.json",
        "cycle_registry": "framework/concordloom/v6/cycle-registry.json",
        "policy": "framework/concordloom/v6/policy.json",
    }
    extras = {
        "atlas_input": (
            "framework/concordloom/v6/development-model.json", model,
        ),
        "evolution_history": (
            "framework/concordloom/v6/evolution-proposal.json", evolution,
        ),
    }
    proposal = create_binding_proposal(
        ...
        proposal_id="concordloom-self-binding-v6-proposal",
        created_at=STAMP,
    ...
    publication_route = [
        ...
        {
            "node_id": "publish-source-change",
            "loop_id": "publish-source-change",
            "role": "publisher",
            "skill_intent": "github:yeet",
            "model_intent": "none",
            "reasoning_intent": "publish only the pinned branch and pull request",
            "subagent_intent": [],
            "scope": deepcopy(source_scope),
        },
        {
            "node_id": "accept-source-change",
            "loop_id": "accept-source-change",
            "role": "operator",
            "skill_intent": "record exact operator merge decision",
            "model_intent": "none",
            "reasoning_intent": "merge only the independently reviewed candidate",
            "subagent_intent": [],
            "scope": deepcopy(merge_scope),
        },
        {
            "node_id": "maintain-organization-presence",
            "loop_id": "maintain-organization-presence",
            "role": "publisher",
            "skill_intent": "publish the approved organization identity",
            "model_intent": "none",
            "reasoning_intent": "perform only the organization profile effect",
            "subagent_intent": [],
            "scope": deepcopy(organization_scope),
        },
    ]
    ...
    binding = activate_binding(
        ...
        policy,
        activation_decision={
            "decision_id": "activate-concordloom-self-binding-v6",
            "actor": OPERATOR,
            "authority_ref": "operator",
            "accepted_at": "2026-07-27T18:01:00Z",
            "rationale": (
                "Activate the exact source publication and organization "
                "presence successor accepted by the operator. The evolution "
                "proposal did not activate itself."
            ),
        },
        binding_id="concordloom-self-binding-v6",
        extra_artifacts=extras,
    )
    ...
    catalog = append_binding(
        base_catalog,
        binding,
        path="framework/concordloom/v6/binding.json",
    )
    save(TARGET / "binding.json", binding)
    save(SOURCE / "catalog.json", catalog)
    print(
        "SOURCE_PUBLICATION_V6_ACTIVATED "
